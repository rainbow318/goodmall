...
type EmptyCartResult struct {
	Success *cart.EmpytCartResp
}

var EmptyCartResult_Success_DEFAULT *cart.EmpytCartResp

func (p *EmptyCartResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(cart.EmpytCartResp)
	}
	return p.Success.FastRead(buf, _type, number)
}
...
func (p *EmptyCartResult) GetSuccess() *cart.EmpytCartResp {
	if !p.IsSetSuccess() {
...
func (p *EmptyCartResult) SetSuccess(x interface{}) {
	p.Success = x.(*cart.EmpytCartResp)
}
...
func (p *kClient) EmptyCart(ctx context.Context, Req *cart.EmptyCartReq) (r *cart.EmpytCartResp, err error) {
