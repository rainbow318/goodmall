...
func AddItem(ctx context.Context, db *gorm.DB, item *Cart) error {
	// 先查看相关商品是否存在于购物车，如果有的话就直接在商品数量上进行改动；如果没有的话就需要新增一条记录
	var row Cart
	...
	// return c.db.WithContext(c.ctx).Model(&Cart{}).Create(&Cart{userId: cart.userId, ProductId: cart.ProductId, Qty: cart.Qty}).Error
